...
$token = isset($_GET['tokenId']) ? MySQL::filter($_GET['tokenId']) : '';
...
//printR($row);
    ...
    if ($token) {
        ...
        switch ($row['type']) {
            ...
            case 0:
                //arhivalas
                MySQL::runCommand('UPDATE training_slideshow_score SET archive=1 WHERE training_id=' . (int)$trainingId . ' AND slideshow_id=' . (int)$showId . ' AND office_id=' . (int)$_SESSION['office_id']);

                //visited rekord beszurasa
                TrainingDetails::addVisited($trainingId,$showId,$_SESSION['office_id'],$_SESSION['u_id'],$_REQUEST['tokenId']);
                ...
                MySQL::execute('INSERT IGNORE INTO training_slideshow_score (training_id,slideshow_id,office_id,u_id,token_id,created,type,testtype,success,archive,visited,rate)
                        VALUES("' . (int)$trainingId . '","' . (int)$showId . '","' . (int)$_SESSION['office_id'] . '","' . (int)$_SESSION['u_id'] . '","' . MySQL::filter($_REQUEST['tokenId']) . '",NOW(),'.$typeResult['type'].','.$typeResult['testtype'].',1,0,0,0)
                    ');
                break;
            //TESZT ESETEN A REGI VISITED ERTEKEK ARCHIVALASA, ES UJ VISITED REKORD BESZURASA
            case 1:
                //archivalas
                MySQL::runCommand('UPDATE training_slideshow_score SET archive=1 WHERE visited=1 AND training_id=' . (int)$trainingId . ' AND slideshow_id=' . (int)$showId . ' AND office_id=' . (int)$_SESSION['office_id']);
...
////////////////////////////////////////////////////////////
//if slideshow is test and last test slide is absolved
//generate a Thank You slide
// IMPORTANT on thanku slide refresh, token is last
//////////////////////////////////////////////////////////////
//$token = "id1384907070872";
//$formArray0['training_id'] = 18;
if($badge === 'end'){
//printR($_GET);
//printR( $token);
	...
	// get current training result -> credit
	$sql = "SELECT credit FROM training_training WHERE training_id = ".(int)$formArray0['training_id'];
    $rows = MySQL::query($sql,false, false);
	$trainingCredit = $rows[0]['credit'];
//printR($rows);

	//get training_training credit
	//$sql = "SELECT credit FROM training_training WHERE training_id = ".(int)$formArray0['training_id'];
    //$rows = MySQL::query($sql,false, false);
	//or count slideshow slides slidelevel
	$sql_levelcount = 'SELECT SUM(slideLevel) AS level FROM slide_slides WHERE slideshow_id = '.(int)$showId;
	$rows_levelcount = MySQL::query($sql_levelcount,false, false);
	$maxSlideshowPoint = (int)$rows_levelcount[0]['level']*100;
//printR($maxSlideshowPoint);

	//count training_slideshow_score with given token id
	$sql2 = 'SELECT SUM(result) AS point FROM training_results WHERE training_id = '.(int)$formArray0['training_id'].' AND u_id = "'.$_SESSION['u_id'].'" AND token = "'.$token.'"';
	$rows2 = MySQL::query($sql2,false, false);
//printR($rows2);
//exit;

	$resultInPercent = $rows2[0]['point']/$maxSlideshowPoint*100;
	$resultInCredit = $trainingCredit*$resultInPercent/100;
	
	// set slide data
	$htmlForSlideshow = '<div class="slideItem" style="left: 0.26666666666666666%; top: 10%; width: 99%; height: auto; position: absolute;"><h1 style="text-align:center">Thank you for your time!</h1></div>';
	if($resultInPercent >51){
		$htmlForSlideshow .= '<div class="slideItem" style="left: 0.26666666666666666%; top: 30%; width: 99%; height: auto; position: absolute;"><h2 style="text-align:center">You\'ve done it successfully</h2></div>';
	} else {
		$htmlForSlideshow .= '<div class="slideItem" style="left: 0.26666666666666666%; top: 30%; width: 99%; height: auto; position: absolute;"><h2 style="text-align:center">You\'ve failed it</h2></div>';
	}
	$htmlForSlideshow .= '<div class="slideItem" style="left: 0.26666666666666666%; top: 40%; width: 99%; height: auto; position: absolute;"><h2 style="text-align:center">Your result is: '.$resultInPercent.'%</h2></div>';
	$htmlForSlideshow .= '<div class="slideItem" style="left: 0.26666666666666666%; top: 50%; width: 99%; height: auto; position: absolute;"><h2 style="text-align:center">Your credit is: '.(int)$resultInCredit.'</h2></div>';
	$returnData[] = array(
            'id' => 'thankuslide',
            'type' => 'normal',
            'templateType' => 'normal',
            'html2' => htmlspecialchars_decode($htmlForSlideshow), //stripslashes($row['html']),//base64_decode($row['html']),
            'tag' => '',
            'badge' => 1,
            'lft' => 2,
            'description' => '',
            'templateOption' => '',
            'background' => 'transparent' //$row['background'] == NULL ? 'transparent' : $row['background']
        );
} else {
	//$badge = 20;
	////////////////////////////////////////////////////////////
...
<body class="fade firstTime <?= $_SESSION['office_nametag']; ?>">
...
<div id="impress">
	...
	if($returnData[0]['badge'] == 1 && $returnData[0]['templateType'] == 'normal'){
...
/*
    if ($badge < 3){
        echo '<div id="overview" class="step slide" data-x="3000" data-y="0" data-scale="10">
                <h1>'.$title.'</h1>
                <h3>'.$author.'</h3>
              </d>';
    }
...
if($returnData[0]['templateType'] == 'normal' && $badge !== 'end' && $db>1) {
...
if ( event.origin !== window.location.host ) return
    ...
    $(function () {
        ...
        $.when(
				...
				$('div[class^="pointer-right"]').bind('click', function(){ impress().next(); });
				$('div[class^="pointer-left"]').bind('click', function(){ impress().prev(); });
