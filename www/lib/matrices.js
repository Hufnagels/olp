...
/*(function() {*/
  ...
  if (!node.addEventListener) {
    node.attachEvent(type, listener);
}
else {
    ...
    $editedobject = $('.slideItem.elementSelected');
		this.mirrored = $editedobject.attr('data-matrix');
    // !/(varsoft)/.test(window.location.href);
...
console.log("------- " +$editedobject.attr("id"));
      ...
      // temp transformation during move, added to display on release
		...
		handleMousemove: function(e){
			...
			this.current = matrix;
			targetobject = $('.slideItem.elementSelected')[0];
...
console.log(this.play)
      ...
      Style.setTransform(targetobject, matrix)
		...
		save: function() {
			...
			this.state = state;
      $editedobject.attr('data-matrix','#'+state);
alert(state);
			if(!this.mirrored) {
      ...
      $editedobject.attr('data-matrix','#'+state);
		 ...
		 */
		restore: function() {
			var hash =  $editedobject.attr('data-matrix');//window.location.hash;			
			if(hash && hash.length) {
				try {
...
/**/
				var mat = $("#play").css(prefix+"transform");
...
//console.log( 'prefix: '+prefix );
				$editedobject.css(prefix+"transform", mat);
				$editedobject.css(prefix+"transform-origin", "0px,0px");
				var tr = mat;
