...
//$imageURL = 'media.'.DOMAINTAG.'/'.$_SESSION['office_nametag'].'/';
//clearstatcache();
...
$result = $m->query($sql,false,false);
      ...
      <div class="affix-top" id="myMediaBoxesContainer"><!-- myMediaBoxesContainer -->
        ...
        <div class="sideBar span2">
          <ul class="mediaBoxList span2" id="mediaBoxList" data-object-type="mediaBoxList">
            <!-- viewAll -->
            <li class="viewAll mbcholder selected" data-object-id="viewAll"><div class="mbHeader"><div class="name">view all</div><div class="pointer-right"></div></div></li>
            <!-- /viewAll -->

          </ul>
        </div>
        <!-- /mediagroups -->
      </div><!-- /myMediaBoxesContainer -->
      <!-- trash --><!--
      ...
      <div id="myMediaFiles">
        ...
        <div class="clearfix"></div>
          ...
          <div id="content" class="closed transitions middlegreyB">
            <div class="inner">
                <!-- UPLOAD FORM -->
              <form id="fileupload" action="/assets/fileupload/server/php/" method="POST" enctype="multipart/form-data">
            ...
            <div class="pull-left">
              <h3 class="functionHeader middlegreyT2">show:
                <div id="filters" class="btn-group" data-toggle="buttons-checkbox" isotope-data-option-key="filters">
                    <button type="button" class="btn btn-dark active" isotope-data-filter="all"><span>show all</span></button>
            ...
            <div class="pull-left">
              <h3 class="functionHeader middlegreyT2">sort:
                <div id="sortBy" class="btn-group" data-toggle="button-radio" isotope-data-option-key="sortBy">
                  <button type="button" class="btn btn-dark active" isotope-data-option-value="original-order"><span>name</span></button>
                  <!--<button type="button" class="btn btn-dark " isotope-data-option-value="name"><span>name</span></button>-->
                  <button type="button" class="btn btn-dark " isotope-data-option-value="number"><span>date</span></button>
                </div>
                <div id="sortOrder" class="btn-group" data-toggle="" isotope-data-option-key="sortAscending">
                  <button type="button" class="btn btn-dark active" isotope-data-option-value="true"><span><i class="icon-arrow-up"></i></span></button>
                  <button type="button" class="btn btn-dark " isotope-data-option-value="false"><span><i class="icon-arrow-down"></i></span></button>
                </div>
              </h3>
            </div>
            <?
            ...
            <div class="pull-right">
              <h3 class="">
                <div class="btn-group dropdown">
                  ...
                  <li class=""><a href="#" onclick="collectDownload()">Download selected</a><li>
                  <li class=""><a href="#" onclick="$('#myMediaList li').toggleClass('selectedElement');($('#selectAll').hasClass('active') ? $('#selectAll').removeClass('active').find('span').text('Select all'):'');">Invert selection</a><li>
                  <li class=""><a href="#" onclick="$('#myMediaList li').removeClass('selectedElement');($('#selectAll').hasClass('active') ? $('#selectAll').trigger('click'):'');">Deselect all</a><li>
                  </ul>
                </div>
              </h3>
            </div>
...
$(function () {
  ...
  $('#fileupload').fileupload({url: '/assets/fileupload/server/php/',filesContainer: $('.files1')});
...
/*
  ...
  $('#fileupload').fileupload(
    'option',
    'redirect',
    window.location.href.replace(/\/[^\/]*$/,'/assets/fileupload/cors/result.html?%s')
  );

  if (window.location.hostname === 'skillbi.local') {

    $('#fileupload').fileupload('option', {
      url: '/assets/fileupload/server/php/',
      maxFileSize: 10000000,
      // The maximum width of the preview images:
      previewMaxWidth: 160,
      // The maximum height of the preview images:
      previewMaxHeight: 120,
      acceptFileTypes: /(\.|\/)(gif|jpe?g|png|pdf|xls?|doc?|swf|ogg|flv|mp4|mp3)$/i,
      process: [
        {
        action: 'load',
        fileTypes: /^image\/(gif|jpeg|png)$/,
...
//teszt
  ...
  $('#myMediaBoxesContainer').affix({
    offset: {
      top: function () { return $window.width() <= 1200 ? 150 : 180 },
      bottom: 10
    }
  });
...
////console.log( boxid );
    ...
    $('.sub').droppable(mediaBoxDropOption);
...
////console.log( video.provider + ' ' + video.id +' videores: ' + videoResult.title);
  ...
  $('#uploadFinish').bind('click', function(){
    if($('ul.files1 li.template-download').length == 0) return false
    var data = [];
    data['result'] = [];
...
*/
    $(this).filter(function(){ 
        return !$(this).hasClass('error') ? data['result'].push({'type':$(this).attr('isotope-data-category'),'mediatype':$(this).attr('data-mediatype'),'name': $(this).attr('isotope-data-name'), 'uploaded_ts':$(this).attr('isotop-data-uploaded'),'uploaded':$(this).find('span.uploaded').text(),'thumbnail_url':$(this).find('img').attr('src'),'mediaurl':$(this).attr('data-mediaurl'),'additional':$(this).find('span.additional').text()}):'';
      });
  ...
  $('#selectAll').bind('click', function(){
    var elements = $('#myMediaList li').filter(function(){ return ($(this).hasClass("isotope-hidden") == false) });
      ...
      <div class="thumbnail ">
        ...
        <div class="caption">
          ...
          {% } else if (o.files.valid && !i) { %}
              ...
              <p><div class="progress progress-success progress-striped active" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><div class="bar" style="width:0%;"></div></div></p>
              <span class="start">
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
     <li class="template-download {% if (file.error) { %}error {% } %}span2 lightgreyB "
        ...
        isotope-data-name="{%=file.name%}"
        isotop-data-uploaded="{%=file.uploaded%}"
        data-mediaurl="<?=connectionType().$imageURL;?>{%=file.mediaurl%}"
        data-mediatype="{%=file.mediatype%}">
        {% if (file.error) { %}
        ...
        <div class="colorBar {%=file.typeShort%}"></div>
        <div class="thumbnail">
          {% if (file.thumbnail_url) { %}<img src="<?=connectionType().$imageURL;?>{%=file.thumbnail_url%}" width="160" height="120">{% } %}
...
{% for (var i=0, file; file=o.result[i]; i++) { %}
 ...
 <form class="form-horizontal">
   ...
   <div class="control-group">
     ...
     <div class="controls"><textarea id="ytDesc" style="margin: 0px; width: 316px; height: 107px;"></textarea></div>
   </div>
...
{% for (var i=0, file; file=o.result[i]; i++) { %}
  <video controls preload="auto" width="100%" height="100%" poster="{%=file.poster%}">
...
<table class="table table-striped table-condensed">
  ...
  <thead><tr><th>Name</th><th>in Mediabox</th></tr></thead>
  <tbody>
    {% for (var i=0, file; file=o.result[i]; i++) { %}
    <tr>
...
<table class="table table-striped table-condensed" style="width:100%;">
  ...
  <thead><tr><th style="width:20%;">Name</th><th style="width:10%;">badge</th><th style="width:25%;">tag</th><th style="width:45%;">description</th></tr></thead>
  <tbody>
    {% for (var i=0, file; file=o.result[i]; i++) { %}
    <tr>
      <td >{%=file.name%}</td>
      ...
      {% for (var j=0, slide; slide=file.slides[j]; j++) { %}
        <tr><td style="width:10%";>{%=slide.badge%}</td><td style="width:35%">{%=slide.tag%}</td><td style="width:55%";>{%=slide.description%}</td></tr>
      {% } %}
      </table>
      </td>
    </tr>
...
<table class="table table-striped table-condensed" style="width:100%;">
  ...
  <tr><td colspan="4">{%=o.result.error%}</td></tr>
...
<table class="table table-striped table-condensed" style="width:100%;">
  ...
  <thead><tr><th style="width:85%;">Name</th><th style="width:15%;">Type</th></tr></thead>
  <tbody>
  {% for (var j=0, file; file=o[j]; j++) { %}
  <tr><td style="width:85%">{%=file.name%}</td><td style="width:15%">{%=file.type%}</td></tr>
  {% } %}
  </tbody>
</table>
</div>
